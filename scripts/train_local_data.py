#!/usr/bin/env python3
"""Training with local 2638 high-quality LeetCode problems + rotating eval sets."""
...
class LocalLeetCodeDataset(Dataset):
    """Load local high-quality LeetCode problems."""
    ...
    def __getitem__(self, idx):
        ...
        # Format: problem description + starter code as prompt, completion as target
...
def run_eval(model, tokenizer, eval_indices, jsonl_path, max_problems=20):
    """Run evaluation on rotating set of problems."""
    model.eval()

    # Sample from eval_indices if more than max_problems
    if len(eval_indices) > max_problems:
        sample_indices = random.sample(eval_indices, max_problems)
    else:
        sample_indices = eval_indices

    dataset = LocalLeetCodeDataset(tokenizer, jsonl_path, indices=sample_indices)
    ...
    for batch in loader:
        ...
        try:
            ...
            # Simple heuristic: if output has valid Python, count as pass
...
def main():
    ...
    batch_size = 1
    lr = 2e-4
    grad_accum = 1  # Reduced from 2
    max_steps = 50000
    log_every = 10
    save_every = 200
    eval_every = 1000
    target_hour, target_min = 6, 0
    early_stop_accuracy = 95.0
    eval_size = 20
    ...
    # Split: 80% train, 20% eval (we'll rotate eval)
    ...
    print(f"Running until {target_hour:02d}:{target_min:02d} ({hours_avail:.1f}h, ~{max_steps} steps)")
    print(f"Using local high-quality data with rotating eval sets")
    ...
    teacher = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="cpu",  # Load on CPU to save GPU VRAM
    ...
    print(f"Data: {len(train_indices)} train | Eval: {len(eval_indices)} (rotate {eval_size})")
    print(f"{'=' * 60}\n")
    ...
    try:
        for epoch in range(100):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                lr_now = lr * lr_fn(step + start_step)
                ...
                # Get teacher logits on CPU to save GPU memory
                with torch.no_grad():
                    input_ids_cpu = input_ids.cpu()
                    attention_mask_cpu = attention_mask.cpu()
                    teacher_logits = teacher(
                        input_ids=input_ids_cpu, attention_mask=attention_mask_cpu
                    ).logits.to("cuda")
                ...
                if (batch_idx + 1) % grad_accum == 0:
                    ...
                    if step % log_every == 0:
                        ...
                        eta_min = (max_steps - step) / sps / 60 if sps > 0 else 0
                    ...
                    if step % save_every == 0:
                        ckpt_path = os.path.join(output_dir, f"step_{start_step + step}.pt")
                        torch.save({
                            "step": step,
                    ...
                    if step > 0 and step % eval_every == 0:
                        print(f"\n{'=' * 60}")
                        print(f"Eval at step {step} (rotating set of {eval_size} problems)")
                        print(f"{'=' * 60}")
                        student.eval()
                        eval_results = run_eval(
                            student, tokenizer, eval_indices,
                            local_data_path, max_problems=eval_size
                        )
                        print(f"  Accuracy: {eval_results['passed']}/{eval_results['total']} ({eval_results['accuracy']:.1f}%)")

                        if eval_results["accuracy"] >= early_stop_accuracy:
                            print(f"\nReached {eval_results['accuracy']:.1f}% accuracy!")
    ...
    # Final save
    ckpt_path = os.path.join(output_dir, "final_local.pt")
    torch.save({
        "step": start_step + step,
        "model": student.state_dict(),
        "config": config.to_dict(),
    }, ckpt_path)
    print(f"\nFinal saved: {ckpt_path}")
    print(f"Total: {step} steps this run ({start_step + step} cumulative)")
